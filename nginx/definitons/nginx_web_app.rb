define :nginx_web_app, :template => "site.erb", :enable => true do
  ...
  file "#{node[:nginx][:dir]}/ssl/dhparam.pem" do
    cookbook 'nginx'
    owner "root"
    group "root"
    mode 0600
    ...
    key_size = 4096
    key_generator = 3
    log "Generating #{key_length} bit dhparam file, this will take some time, so be patient"
    dhparam_content = OpenSSL::PKey::DH.new(key_length, key_generator).to_pem
